...
survClust can be run over a single or multiple datasets in an integrated fashion across different modalities profiled for constant subjects. According to the nature and quality of these datasets, the integrated solution might perform better or worse by incorporating all the available information. We ran survClust on individual datasets from scenarios 2 and 3 and compared them with the integrated run as reported above. 
...
In the plots above, we plot the median value across cross validated rounds for each k for that particular metric (logrank, SPWSS etc.). The bars represent the 25th and 75th percentiles of the data. 

In scenario2, dataset2 was simulated as a noisy dataset with very weak clustering information. In the plots above dataset2 performs worse than integrated solution and dataset1 alone. Dataset1 was simulated with a strong clustering information and is doing as well as integrated survClust solution in terms of peak logrank at `k=3`. 

A similar trend is seen for scenario 3, where dataset1 had strong cluster separation but poor survival association with overall low median logrank values. Whereas, integrated solution and dataset2 alone are performing similar. 
